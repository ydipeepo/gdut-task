class_name GDUT_AllSettledTask extends CustomTask

#-------------------------------------------------------------------------------
#	METHODS
#-------------------------------------------------------------------------------

static func create(init_array: Array, name := &"Task.all_settled") -> Task:
	#
	# 事前チェック
	#
	...
	if is_pending:
		if task is Task:
			match task.get_state():
				STATE_COMPLETED:
					result_set[index] = GDUT_CompletedTask.create(result)
					_pending -= 1
					if _pending == 0:
						release_complete(result_set)
				STATE_CANCELED:
					result_set[index] = GDUT_CanceledTask.create()
					_pending -= 1
					if _pending == 0:
						release_complete(result_set)
				_:
