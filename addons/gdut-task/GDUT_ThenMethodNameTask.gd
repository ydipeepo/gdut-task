...
static func create(
	...
	match method_argc:
		0:
			if not GDUT_Task.is_valid_task_then_method_name_0(object, method_name):
				push_error(GDUT_Task.get_message(&"BAD_METHOD_ARGS", method_name))
				return GDUT_CanceledTask.create(name)
		1:
			if not GDUT_Task.is_valid_task_then_method_name_1(object, method_name):
				push_error(GDUT_Task.get_message(&"BAD_METHOD_ARGS", method_name))
				return GDUT_CanceledTask.create(name)
		2:
			if not GDUT_Task.is_valid_task_then_method_name_2(object, method_name):
				push_error(GDUT_Task.get_message(&"BAD_METHOD_ARGS", method_name))
				return GDUT_CanceledTask.create(name)
		_:
