class_name Task_Index extends Test
...
func カスケード_CustomTask_派生() -> void:
	...
	if not is_not_empty(opers2):
		return
	var task := Task.index(opers1[0], opers2[0])
...
func カスケード_Task_派生() -> void:
	...
	var task3 := Task.index(task1, task2)
...
func カスケード_Task_派生_キャンセルあり_即時() -> void:
	...
	are_equal(0, await task3.wait(Cancel.deferred()))
...
func 状態遷移_入力_即時_ラップされたリテラル() -> void:
	var task := Task.index(Task.completed(12), Task.completed(34), Task.completed(56))
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(0, await task.wait())
...
func 状態遷移_入力_混合_ラップされたリテラル() -> void:
	...
	var task := Task.index(callsite.noop_return, callsite.fork_return, callsite.noop_return)
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(0, await task.wait())
...
func 状態遷移_入力_遅延_ラップされたリテラル() -> void:
	var callsite := Callsite.new(self)
	var task := Task.index(callsite.fork_return, callsite.fork_return, callsite.fork_return)
	if not is_not_null(task):
		return
	is_true(task.is_pending)
	are_equal(0, await task.wait())
...
func 状態遷移_入力_即時_キャンセル含む_リテラル() -> void:
	var task := Task.index(12, 34, Task.canceled())
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(0, await task.wait())
...
func 状態遷移_入力_即時_キャンセル含む_ラップされたリテラル() -> void:
	var task := Task.index(Task.completed(12), Task.completed(34), Task.canceled())
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(0, await task.wait())
...
func 状態遷移_入力_混合_キャンセル含む_ラップされたリテラル() -> void:
	var callsite := Callsite.new(self)
	var task := Task.index(callsite.noop_return, callsite.fork_return, Task.canceled())
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(0, await task.wait())
...
func 状態遷移_入力_遅延_キャンセル含む_ラップされたリテラル() -> void:
	var callsite := Callsite.new(self)
	var task := Task.index(callsite.fork_return, callsite.fork_return, Task.canceled())
	if not is_not_null(task):
		return
	is_true(task.is_pending)
	are_equal(0, await task.wait())
