class_name Task_Count extends Test
...
func カスケード_CustomTask_派生() -> void:
	...
	if not is_not_empty(opers2):
		return
	var task := Task.count(opers1[0], opers2[0])
...
func カスケード_Task_派生() -> void:
	...
	var task3 := Task.count(task1, task2)
...
func 状態遷移_入力_空_キャンセルあり_即時() -> void:
	...
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(0, await task.wait(Cancel.canceled()))
	is_true(task.is_completed)
...
func 状態遷移_入力_空_キャンセルあり_遅延() -> void:
	...
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(0, await task.wait(Cancel.deferred()))
	is_true(task.is_completed)

func 状態遷移_入力_リテラル() -> void:
	var task := Task.count(12, 34, 56)
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(3, await task.wait())
	is_true(task.is_completed)
...
func 状態遷移_入力_リテラル_キャンセルあり_即時() -> void:
	...
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(3, await task.wait(Cancel.canceled()))
	is_true(task.is_completed)
...
func 状態遷移_入力_リテラル_キャンセルあり_遅延() -> void:
	...
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(3, await task.wait(Cancel.deferred()))
	is_true(task.is_completed)

func 状態遷移_入力_即時_ラップされたリテラル() -> void:
	var task := Task.count(Task.completed(12), Task.completed(34), Task.completed(56))
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(3, await task.wait())
...
func 状態遷移_入力_混合_ラップされたリテラル() -> void:
	...
	var task := Task.count(callsite.noop_return, callsite.fork_return, callsite.noop_return)
	if not is_not_null(task):
		return
	is_true(task.is_pending)
	are_equal(3, await task.wait())
...
func 状態遷移_入力_遅延_ラップされたリテラル() -> void:
	var callsite := Callsite.new(self)
	var task := Task.count(callsite.fork_return, callsite.fork_return, callsite.fork_return)
	if not is_not_null(task):
		return
	is_true(task.is_pending)
	are_equal(3, await task.wait())
...
func 状態遷移_入力_即時_キャンセル含む_リテラル() -> void:
	var task := Task.count(12, 34, Task.canceled())
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(2, await task.wait())
	is_true(task.is_completed)
...
func 状態遷移_入力_即時_キャンセル含む_リテラル_キャンセルあり_即時() -> void:
	...
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(2, await task.wait(Cancel.canceled()))
	is_true(task.is_completed)
...
func 状態遷移_入力_即時_キャンセル含む_リテラル_キャンセルあり_遅延() -> void:
	...
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(2, await task.wait(Cancel.deferred()))
	is_true(task.is_completed)

func 状態遷移_入力_即時_キャンセル含む_ラップされたリテラル() -> void:
	var task := Task.count(Task.completed(12), Task.completed(34), Task.canceled())
	if not is_not_null(task):
		return
	is_true(task.is_completed)
	are_equal(2, await task.wait())
...
func 状態遷移_入力_混合_キャンセル含む_ラップされたリテラル() -> void:
	var callsite := Callsite.new(self)
	var task := Task.count(callsite.noop_return, callsite.fork_return, Task.canceled())
	if not is_not_null(task):
		return
	is_true(task.is_pending)
	are_equal(2, await task.wait())
...
func 状態遷移_入力_遅延_キャンセル含む_ラップされたリテラル() -> void:
	var callsite := Callsite.new(self)
	var task := Task.count(callsite.fork_return, callsite.fork_return, Task.canceled())
	if not is_not_null(task):
		return
	is_true(task.is_pending)
	are_equal(2, await task.wait())
